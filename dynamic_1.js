...
(function(){
	...
	var value_arr = [1, 1.6, 3.3, 4.1, 7.2, 9.0, 9.2, 10.1, 9.5, 8.1];
	var splitee = 45;
	utils.print("max value is", dynamic_1(splitee, piece_arr, value_arr));
...
function dynamic_1(total, pieces, values){
	var cache = {};
	var split_cache = {};
	var iter_times = 0;
	var inner = function(total){
		...
		for (var i = 0, len = pieces.length; i < len; i++){
			if (total >= pieces[i]){
				var t = values[i] + inner(total - pieces[i]);
